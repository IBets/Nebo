import x_math.angles
import x_math.vector as vector
import x_math.constant_tools as constant
import math
import numpy as np
...
class Mat3:
    ...
    def to_Angles(self):
        ...
        s = math.sqrt(self._m[0][0] * self._m[0][0] + self._m[0][1] * self._m[0][1])

        ang = x_math.angles.Angles(0.0, 0.0, 0.0)
        atan = math.atan2

        if s > constant.FLT_EPSILON:
            ang.pitch = -atan(self._m[0][2], s) * constant.M_RAD2DEG
            ang.yaw = atan(self._m[0][1], self._m[0][0]) * constant.M_RAD2DEG
            ang.roll = atan(self._m[1][2], self._m[2][2]) * constant.M_RAD2DEG
        else:
            ang.pitch = self._m[0][2] < 0.0 if 90.0 else -90.0
            ang.yaw = -atan(self._m[1][0], self._m[1][1]) * constant.M_RAD2DEG
            ang.roll = 0.0
